...
async function cleanupRegistry() {
  ...
  try {
    ...
    const registryItemNames = new Set(
      registry.items.filter((item) => !RESERVED_NAMES.has(item.name)).map((item) => item.name)
    );

    console.log(`📋 Found ${registryItemNames.size} items in registry TypeScript files`);
    ...
    console.log(`📁 Found ${jsonFiles.length} JSON files in registry directory`);
